...
function applyPrefill(setValues, setPlanInterest, detail) {
  ...
  setValues((v) => ({
    ...
    service: detail?.service ?? v.service,
    courseId: detail?.courseId ?? v.courseId,
    algoAddon: detail?.algoAddon ?? v.algoAddon,
  }));
}
...
export default function LeadForm() {
  ...
  useEffect(() => {
    if (location.state?.prefill) {
      applyPrefill(setValues, setPlanInterest, location.state.prefill);
      // Clear it so navigating back/forward or refreshing doesn't re-apply it.
  ...
  useEffect(() => {
    function handlePrefill(e) {
      applyPrefill(setValues, setPlanInterest, e.detail);
    }
    ...
    return (
      ...
      >
        ...
        <p className="max-w-sm text-sm text-mist/60">
          Our team will get back to you soon. We usually respond within 1–2
          business days.
        </p>
        <button
    ...
    <form onSubmit={handleSubmit} noValidate className="card p-6 md:p-10">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          <label
            htmlFor="name"
            className="mb-1.5 block text-xs font-medium text-mist/50"
          >
            Name
          </label>
          <input
          ...
          />
          {errors.name && (
            <p className="mt-1.5 text-xs text-loss">{errors.name}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.email && (
            <p className="mt-1.5 text-xs text-loss">{errors.email}</p>
          )}
        </div>

        <div>
          <label className="mb-1.5 block text-xs font-medium text-mist/50">
            Phone Number
          </label>
          <PhoneInput
        ...
        <div>
          ...
          />
          {errors.city && (
            <p className="mt-1.5 text-xs text-loss">{errors.city}</p>
          )}
        </div>
        ...
        {values.service === "Course" && (
          <div className="md:col-span-2 rounded-lg border border-mist/10 bg-ink-800/40 p-4">
            <p className="mb-3 text-xs font-medium text-mist/50">
              Which course?
            </p>
            ...
            {errors.courseId && (
              <p className="mt-1.5 text-xs text-loss">{errors.courseId}</p>
            )}
      ...
      {status === "error" && (
        <p className="mt-4 text-sm text-loss">
          Something went wrong submitting your request. Please try again in a
          moment.
        </p>
      )}
